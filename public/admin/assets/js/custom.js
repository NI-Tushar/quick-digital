$(document).ready(function () {
    ...
    //user
$("#current_password_user").keyup(function () {
    var current_password_user = $("#current_password_user").val();
    $.ajax({
        headers: {
            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
        },
        type: 'post',
        url: '/user/check_current_password',
        data: { current_password_user: current_password_user },
        beforeSend: function () {
            $("#verifyCurrentPassword").html("Checking...");
        },
        success: function (resp) {
